# canvas_iterative_deconv_v2_fixed.py
...
st.set_page_config(page_title="Iterative Kern-Zählung (OD + Deconv) — V2 Fixed", layout="wide")
st.title("🧬 Iterative Kern-Zählung — V.2 (Fixed)")
...
with col2:
    ...
    detection_threshold = st.sidebar.slider("Threshold (0-1) für Detektion", 0.01, 0.9, 0.2, 0.01)
    min_area_display = st.sidebar.number_input("Min. Konturfläche (px) — angezeigt", min_value=1, max_value=2000, value=80)
...
if coords:
    ...
    if mode == "Punkt löschen":
        ...
        if vec is None:
            ...
            # ----- FIX: Kalibrierpunkt immer mitzählen -----
            if not any(is_near((x_orig, y_orig), p, dedup_dist_orig) for p in st.session_state.all_points):
                centers_orig.insert(0, (x_orig, y_orig))
